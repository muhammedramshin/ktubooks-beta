...
export class ContactsComponent implements OnInit {
  ...
  getcontacts(){
    this.afs.collection('contacts').valueChanges({idField:'id'}).subscribe((res:any) => {
      console.log(res);
...
editRecord(event:any){
  ...
  this.afs.collection('contacts').doc(event.data.id).update({
    name: event.newData.name,
    contact: event.newData.contact,
    updatedAt: new Date(),
  }).then((res:any) => {
...
deleteRecord(event:any){
  console.log(event);
  event.confirm.resolve(event.newData);
  this.afs.collection('contacts').doc(event.data.id).delete().then((res:any) => {
    console.log(res);
    event.confirm.resolve(event.newData);
  }
  ).catch((err:any) => {
